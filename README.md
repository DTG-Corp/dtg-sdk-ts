...
A full reference for this library is available [here](https://github.com/DTG-Corp/dtg-sdk-ts/blob/HEAD/./reference.md).
...
try {
    ...
    if (err instanceof DtgAgentSdkError) {
...
```typescript
const { data, rawResponse } = await client.gateway.createChatCompletion(...).withRawResponse();
